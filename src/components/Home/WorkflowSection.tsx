...
export default function WorkflowSection() {
  const [d, setD] = useState<WorkflowData>(defaults);
  const router = useRouter();

useEffect(() => {
  fetchWithCache("workflow_content", () =>
    new Promise((resolve) => {
  ...
  const stepClass = "cursor-pointer transition-all duration-300";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          ...
          <motion.div
            ...
            className={`${stepClass} flex items-center gap-6 bg-[#F5F5F3] border border-black/15 rounded-3xl px-6 py-6 md:px-8 md:py-7`}
          >
            ...
            <div className="hidden md:flex w-10 h-10 rounded-full border-2 border-black items-center justify-center shrink-0 transition-colors duration-200 hover:bg-black hover:text-white">
          ...
          <motion.div
            ...
            onClick={() => router.push("/contact")}
            whileHover={{ scale: 1.01, boxShadow: "0 8px 32px rgba(190,242,100,0.4)" }}
            className={`${stepClass} flex items-center gap-6 bg-[#BEF264] border border-black/10 rounded-3xl px-6 py-6 md:px-8 md:py-7`}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[3, 4].map((n, i) => (
              <motion.div
                key={n}
                initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 + i * 0.1 }}
                onClick={() => router.push("/contact")}
                whileHover={{ scale: 1.02, boxShadow: "0 8px 32px rgba(0,0,0,0.10)" }}
                className={`${stepClass} flex flex-col gap-4 bg-[#F5F5F3] border border-black/15 rounded-3xl px-6 py-7 md:px-8 md:py-8`}
              >
